	...
	<div class="container">
		<div class="waterfall-grid">
			<transition-group name="stagger">
				<div v-for="(img, index) in images" :key="img.id" class="waterfall-item"
					:style="{ '--aspect-ratio': img.aspectRatio }" @click="openViewer(index)">
					<!-- 骨架屏 -->
					...
					<!-- 图片主体 -->
					<img :src="imgUrl(img.name)" alt="Gallery image" loading="lazy" class="gallery-image"
						:class="{ loaded: img.loaded }" @load="handleImageLoad(img)" />
				</div>
			</transition-group>
		</div>
		...
		<!-- 全屏预览 -->
		<el-image-viewer v-if="viewerVisible" :url-list="previewList" :initial-index="initialIndex"
			@close="closeViewer" />
	</div>
</template>
